...
def map_role_to_grade(
    ...
    # NOTE (found while converting this endpoint to @db_session, not fixed
    # here — out of scope for this pass): "UNIQUE constraint failed" is a
    # SQLite error string. This app runs on Postgres/psycopg2, whose real
    # unique-violation message is "duplicate key value violates unique
    # constraint", so this except clause's friendly-400 branch below has
    # likely never actually matched — a real duplicate mapping falls
    # through to `raise` (an unhandled 500) instead. Left as-is; worth a
    # follow-up using db.IntegrityError (see other routers' pattern) rather
    # than string-matching the driver's message.
    try:
        require_permission(conn, current_user, "compensation.manage_pay_grades_job_levels_roles")
        inst_id = current_user.get("active_institution_id") or current_user.get("institution_id")
